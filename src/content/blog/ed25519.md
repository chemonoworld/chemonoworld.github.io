...
1. **전체 그룹 위수 ($n$)**: 곡선 존재하는 모든 점들의 총 개수
   ...
   n = 8 \cdot q
   $$
   - 여기서 $8$은 <strong>cofactor</strong>라고 부름.
...
<strong>Ed25519</strong>라는 이름은 유한체를 정의하는 소수 $p$에서 유래함.
...
- 공개키를 만들기 위한 <strong>Private Key</strong>로 사용됨.
...
임시 비밀값 $r$은 $H_{high}$와 메시지 해시 $M$으로부터 <strong>결정론적(Deterministic)</strong>으로 생성됨. (RFC-6979와 유사)
...
서명 $\sigma$는 $\text{R}$ (32bytes)과 $s$ (32bytes)를 순서대로 연결한 <strong>64바이트</strong>임.
...
다음 <strong>타원 곡선 방정식</strong>이 성립하는지 확인함.
...
- 하지만 타원곡선 방정식은 $y^2 = x^3 + ax + b$ 형태의 $x$축 대칭성을 가지므로, $x$좌표와 $y$좌표의 <strong>부호(Parity)</strong>만 있으면 점을 완벽히 특정할 수 있음.
- **secp256k1**의 경우:
    - $y$좌표의 부호(Parity)를 나타내는 1바이트 prefix(`0x02` or `0x03`)를 붙여 <strong>33bytes</strong>를 Compressed Public Key로 사용함.
- **Ed25519**의 경우:
    - $y$좌표를 기본으로 저장함.
...
- **X25519**: <strong>키 교환(Key Exchange, ECDH)</strong>을 위해 사용됨. (Montgomery Curve)
